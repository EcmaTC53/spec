...
This specification is intended to be used in strict mode only. Sloppy mode has known issues that detract from building a robust system and is maintained primarily for web compatibility. Sloppy mode provides no benefit to embedded systems.
...
If `buffer` argument has a `bitLength` property, it specifies the number of bits of the buffer to swap in the transfer, overriding the `length` property to allow  transfer of partial bytes.
