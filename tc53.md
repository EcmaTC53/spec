...
<!-- Yeah, I think you've convinced me. I can't come up with a great reason to do async writes from script... if that performance is needed you probably should be 
in native code anyways. And I agree that there's no obviously safe way to do async writes from script in XS without a copy under the hood. 
Let's lose the callback and make this explicitly synchronous (to the host implementation... what happens from there could be a different matter, so I think you still need flush for proper chip select management or for things like a data/command pin.). -AJC  -->

**`transfer(buffer)`**

Write `buffer` to the SPI bus while simultaneously reading `buffer.length` bytes from the SPI bus. The results of the read are placed into `buffer`, replacing the original contents.

The `buffer` argument can have an optional `bitLength` property that specifies the number of bits of the buffer to swap in the transfer. This allows for transfers of partial bytes.
...
<!-- This brings up a slightly different question. Some SPI implementations can (or must) do the transfer operation in-place. The ESP32 spi_device_transmit can do this (and does in modSPITxRx), for instance. So to implement transfer without an extra copy operation, the contents of the send buffer are necessarily replaced with the result of the read. I'm going to try specifying it that way here, with your bitLength suggestion. See what you think. (Put another way: if we specify this as non-destructive to the send `buffer` then some implementations will definitely have to copy that buffer to do the transfer.) -AJC -->
