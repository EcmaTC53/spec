...
To avoid naming collisions with bare module specifiers, a namespace prefix has been proposed. This specification will use a single namespace prefix (currently "tc53:" but expected to change). For example,
...
Classes confirming to the Base Class Pattern may be subclassed.
...
|  `selectActiveValue` | Must be 1 or 0. Specifies what value will be written to the `select` pin when this SPI instance is active. This property is optional and defaults to 0. 
...
#### Chip Select & Flush
Implementations of the `read`, `write`, and `transfer` methods must ensure that the `select` pin is set to `selectActiveValue` before reading from or writing to the SPI bus. If no `select` pin was specified, the caller is responsible for managing the state of any chip select logic.

Some SPI peripherals require that the chip select pin be released at specific times (for instance, to mark the end of a transaction). Because this operation must be performed synchronously after a `read`, `write`, or `transfer` completes, it must be tied to a synchronous flush of the bus. To accommodate this requirement, the SPI class includes a `flush` method that takes an optional `releaseSelect` argument. If set to true, the chip select pin must be released after the flush completes. 

#### Methods

**`read(byteLength)`**

Read and return `byteLength` bytes from the SPI bus as an `ArrayBuffer`. If a Host Out / Peripheral In pin was specified, an implementation-dependent default value is written to the SPI bus during the `read` operation.

**`write(buffer)`**

Write `buffer` to the SPI bus while ignoring or discarding any data on the Host In / Peripheral Out pin (if one was specified).
...
Synchronously flushes any implementation-specific buffers of the SPI host instance. If the optional `releaseSelect` argument is present and `true`, the chip select pin must be released after the flush completes. 
...
## Pixel Display Class Pattern




## Sensor Data Provence Rules
