...
| `activeLow` | A boolean value indicating if the IO is active low or active high. This property is optional and defaults to `false` for active high. |
...
The `writeRead` method performs a write followed immediately by a read, with a stop bit of 0 between the operations. The first argument is a Byte Buffer. The second argument follows the [behavior](#io-pattern-read) of the IO Class Pattern `read` method for the `"buffer"` data format 
...
| `onHeaders(status, headers, statusText)` | A function to invoke to provide the HTTP status result code, the response headers as a `Map`, and the HTTP status message as a string. The map keys are the header names normalized to lowercase and their values are the header values. This property is optional. |
...
| `retain` | A boolean indicating if a `PUBLISH` message should be retained by the server. This property is for `PUBLISH` messages only. It is optional and defaults to `false`. |
| `duplicate` | A boolean indicating if a `PUBLISH` message is being retransmitted by the client. This property is for `PUBLISH` messages only. It is optional and defaults to `false`. |
...
| `mode` | A string indicating the mode used to access to the file. Values are `"r"`, `"r+"`, `"w"` , and `"w+"`. This property is optional and defaults to `"r"`.
...
| Property | Description |
| :---: | :--- |
| `path` | A string indicating the name of the key-value domain to open. This property is required. 
...
| Property | Description |
| :---: | :--- |
| `partition` | A Flash Partition instance for the partition to be updated. The instance must not be closed until after the update is complete. This property is required. 
...
| `byteLength` | A number indicating the size of the update in bytes that will be written to the partition. This value must be less than or equal to the byte length of the partition. This property is optional and should not be specified if the number of bytes is unknown.
...
The host provider instance should include its root [Directory instance](#storage-directory-class) through its `files` property.
